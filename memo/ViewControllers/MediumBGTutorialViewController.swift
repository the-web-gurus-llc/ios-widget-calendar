...
class MediumBGTutorialViewController: UIViewController, PHPickerViewControllerDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? MediumBGSelectViewController {
                ...
                provider.loadObject(ofClass: UIImage.self) { image, error in
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        if let image = image as? UIImage {
                            ...
                            self.performSegue(withIdentifier: "MediumBGSelect", sender: self)
                        } else {
